...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $query = "DELETE FROM expenses WHERE name = :name AND date = :date AND username = :username";
    $stmt = $pdo->prepare($query);
    $stmt->execute(array(':name' => $name, ':date' => $date, ':username' => $username));
...
<body>
  ...
  <form id="transaction-form" method="POST" action="delete.php">
    Enter item name:
    <input type="text" id="name" name="name" placeholder="Item" required><br>
    Enter date of purchase:<br>
    <input type="date" id="date" name="date" required><br><br>
    <button type="submit" class="btn-add">Delete</button>
  </form>
</div>
</body>
</html>
